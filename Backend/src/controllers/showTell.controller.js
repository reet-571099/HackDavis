...
function generateFeedbackMessage(explanation, deed) {
  ...
  if (
    lowerExplanation.includes("not specific") ||
    lowerExplanation.includes("vague")
  ) {
  ...
  if (
    lowerExplanation.includes("different activity") ||
    lowerExplanation.includes("not matching")
  ) {
    return `Remember, we're looking for: "${deed}". Try again with that specific activity!`;
  }

  if (
    lowerExplanation.includes("not clear") ||
    lowerExplanation.includes("unclear")
  ) {
    return "Speak a little louder and clearer next time so we can understand your awesome deed!";
  }

  if (
    lowerExplanation.includes("incomplete") ||
    lowerExplanation.includes("partial")
  ) {
    return "Tell us the whole story! What happened from start to finish?";
  }

  if (
    lowerExplanation.includes("not related") ||
    lowerExplanation.includes("unrelated")
  ) {
  ...
  try {
    ...
    if (analysis.matches) {
      ...
      await categoryRef.set(
        {
          trueCount:
            (categoryDoc.exists ? categoryDoc.data().trueCount || 0 : 0) + 1,
          falseCount: categoryDoc.exists
            ? categoryDoc.data().falseCount || 0
            : 0,
        },
        { merge: true }
      );
      ...
      await categoryRef.set(
        {
          ...
          falseCount:
            (categoryDoc.exists ? categoryDoc.data().falseCount || 0 : 0) + 1,
        },
        { merge: true }
      );
      ...
      res.status(201).json({
        ...
        verification: {
          ...
          category: deed.category,
          suggestion:
            "Try to take a photo that clearly shows you doing the deed. Make sure the main action is visible!",
        },
      });
    }
  } catch (error) {
...
const saveDoodle = async (req, res) => {
  try {
    ...
    if (!deedId || !doodleData) {
      return res
        .status(400)
        .json({ error: "Deed ID and doodle data are required" });
    }
...
const saveJournalEntry = async (req, res) => {
  try {
    ...
    if (!deedId || !content || !type) {
      return res
        .status(400)
        .json({ error: "Deed ID, content, and type are required" });
    }
    ...
    if (analysis.matches) {
      ...
      res.status(201).json({
        ...
        verification: {
          ...
          category: deed.category,
          suggestion:
            "Try to be more specific about how you completed the deed. For example, if the deed is about helping someone, describe exactly how you helped them.",
        },
      });
    }
  } catch (error) {
...
const saveVoiceEntry = async (req, res) => {
  try {
    ...
    console.log("Received voice entry:", {
      deedId,
      file: file
        ? {
    ...
    res.status(500).json({
      error: "Failed to save voice entry",

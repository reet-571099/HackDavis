...
class GeminiService {
  ...
  async generateDeed(category) {
    try {
      ...
      const randomAspect =
        categoryDetails.aspects[
          Math.floor(Math.random() * categoryDetails.aspects.length)
        ];
      ...
      const antiExampleString =
        promptVariationService.getAntiExampleString(recentDeeds);
      ...
      const categoryExamples = categoryRequirements[category].examples
        ? `Some good examples are: "${categoryRequirements[
            category
          ].examples.join('", "')}"`
        : "";
...
Don't include any other text outside the JSON object.`;
      ...
      if (!validationResult.isValid) {
        console.log(
          `Generated deed did not meet requirements: ${validationResult.reason}. Trying again...`
        );
  ...
  async analyzeAudio(audioBase64, { deed, category }) {
    try {
      ...
      const result = await this.model.generateContent([
        ...
        {
          inlineData: {
            mimeType: "audio/webm", // Chrome typically records in webm format
            data: audioBase64,
          },
        },
      ]);
